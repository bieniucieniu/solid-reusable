 ...
 * ```
 */
export function createDrawer(options: Partial<zag.Props> = {}) {
	options.id ??= createUniqueId();
	const service = useMachine(zag.machine, options as never);
	...
	return {
		...
		SwipeArea(props: DynamicAsProps<"div", zag.SwipeAreaProps>) {
			...
			return (
				<Dynamic
					component={local.as ?? "div"}
					{...api().getSwipeAreaProps({ disabled: local.disabled, swipeDirection: local.swipeDirection })}
					{...rest}
				/>
			);
		},
